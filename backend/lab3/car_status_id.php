...
<head>
  ...
  <title>Car Status Details</title>
  <style>
    table {
      width: 80%;
      margin: 20px auto;
...
try {
  ...
  $car_status_id = $_GET['id'];
  $query = "SELECT * FROM `car_status` WHERE id = $car_status_id;";
  $select_detail = mysqli_query($connection, $query);
  if (mysqli_num_rows($select_detail) > 0) {
    $selection_status_dtl = 1;
    echo '<div class=\'table-container\'>';
    echo "<table><tr><th>ID</th><th>Owner</th><th>Status</th><th>Car ID</th></tr>";
